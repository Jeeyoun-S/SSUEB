  ...
  state: {
    userRule: {
      ...
      password: [
        ...
        (v) =>
          (/^(?=.*[a-zA-z])(?=.*[0-9])(?=.*[~!@$!%*#^?&])(?!.*[^a-zA-z0-9~!@$!%*#^?&]).{10,20}$/).test(v) || "영어, 숫자, 특수문자 포함 10~20자로 입력해 주세요.",
      ],
      name: [
        (v) => !!v || "이름은 필수 입력 사항입니다.",
        (v) => v.length <= 22 || "22자 이하로 입력해 주세요.",
        (v) => (/^[가-힣|a-z|A-Z]{1,22}$/).test(v) || "숫자, 특수문자는 입력 불가능합니다.",
      ],
      phone: [
      ...
      nickname: [
        ...
        (v) => v.length <= 22 || "22자 이하로 입력해 주세요.",
        (v) => (/^[ㄱ-ㅎ|ㅏ-ㅣ|가-힣|a-z|A-Z|0-9]{1,22}$/).test(v) || "특수문자는 입력 불가능합니다.",
      ]
    },
    consultantRule: {
      license: [
        (v) => !!v || "필수 입력 사항입니다.",
        (v) => (/^[0-9a-zA-z-]*$/).test(v) || "숫자, 영문자, -만 입력 가능합니다.",
        (v) => v.length <= 30 || "30자 이하로 입력해 주세요."
      ],
      image: [
    ...
    petRule: {
      petBirth: [
        (v) => !v || (/^\d{4}-(0[1-9]|1[012])$/).test(v) || "생일은 YYYY-MM 형식으로 입력해 주세요."
      ],
      petImage: [
        (v) => v == null || v.length <= 1 || "프로필 사진은 하나만 첨부 가능합니다.",
        (v) =>
          v == null || v.length == 0 || (v.length == 1 && ["image/png", "image/jpeg"].includes(v[0].type)) || "png, jpg 파일만 첨부 가능합니다.",
      ],
      petInfo: [
      ...
      petVariety: [
        (v) => v == null || !v || !(/[{}[\]/?.,;:|)|*~`!^\-_+<>@#$%&\\=('"]/).test(v) || "특수문자 입력은 불가능합니다.",
