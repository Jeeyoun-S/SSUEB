  ...
  <v-expansion-panels multiple>
    ...
    >
      ...
      <v-expansion-panel-title>
        <v-col class="pa-0 pl-2 bold-font" cols="1">{{ idx + 1 }}</v-col>
        <!--번호, 연결하지 않음-->
        <v-col class="pa-0" cols="3">{{ reservation.reservationDate }}</v-col>
        <!--상담일시-->
        <v-col class="pa-0" cols="1">{{ reservation.petType }}</v-col>
        <!--대분류-->
        <v-col class="pa-0" cols="3">{{ reservation.petName }}</v-col>
        <!--애기이름-->
        <v-col class="pa-0">
          <!--별점-->
          <v-rating
            v-model="reservation.reviewGrade"
        ...
        <v-sheet class="d-flex flex-row justify-space-between align-center">
          ...
          <v-hover v-slot="{ isHovering, props }" open-delay="200">
            <v-card
              ...
              class="pa-2 mb-5 d-flex flex-column justify-center align-center"
              variant="outlined"
              height="270"
              width="180"
              rounded="0"
          ...
          <v-hover v-slot="{ isHovering, props }" open-delay="200">
            <v-card
              :elevation="isHovering ? 3 : 0"
              :class="{ 'on-hover': isHovering }"
              class="pa-3 mb-5 d-flex flex-column justify-center"
              variant="outlined"
              height="270"
              width="390"
              rounded="0"
