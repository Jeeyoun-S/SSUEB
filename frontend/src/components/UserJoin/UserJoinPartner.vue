<template>
  <v-form class="form" lazy-validation>
    ...
    <v-text-field
      ...
      required
    ></v-text-field>
    <!-- :disabled="$refs.userPhone.validate()" -->
    <v-btn variant="tonal" rounded="0" @click="phoneAuth()" block
      >휴대폰 인증</v-btn
    >
    ...
    <v-btn variant="outlined" size="large" rounded="0" block
      >회원가입 하기</v-btn
    >
  </v-form>
</template>
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        phone: [
          ...
          (v) =>
            this.valid.phone.test(v) || "'-' 없이 숫자 11자리로 입력해 주세요.",
        ],
      },
      // phoneDisable: true
    };
  ...
  methods: {
    ...
    async phoneDisable() {
      console.log("e");
      // if (this.phone != null) {
      //   // console.log(await this.$refs.userPhone.validate().valid);
      //   return await this.$refs.userPhone.isValidating();
      // } else {
      //   return false;
      // }
    },
