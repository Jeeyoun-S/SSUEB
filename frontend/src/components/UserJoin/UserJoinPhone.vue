<template>
  <v-text-field v-model="userPhone"  class="mb-2" :rules="phoneRules" label="휴대폰 번호" variant="underlined" color="primary" required></v-text-field>
  <v-btn variant="tonal" rounded="0" @click="phoneAuth()" :disabled="phoneDisable || phoneAuthStates" block>{{ phoneAuthMessage }}</v-btn>
  <v-checkbox class="none" v-model="phoneAuthStates" :rules="[(phoneAuthStates) => !!phoneAuthStates || '휴대폰 인증을 진행해 주세요.']" label="Do you agree?"></v-checkbox>
</template>
  ...
  data() {
    return {
      ...
      phoneRules: [
        ...
        v => this.phoneValid.test(v) || "'-' 없이 숫자 11자리로 입력해 주세요."
      ],
    }
  },
  methods: {
