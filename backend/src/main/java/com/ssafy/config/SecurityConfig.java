...
//              .addFilter(new JwtAuthenticationFilter(authenticationManager(), userService)) // HTTP 요청에 JWT 토큰 인증 필터를 거치도록 필터를 추가
                ...
                .authorizeRequests()						// URL 별 인증 관리
       				.antMatchers("/api/user/login/**", "/api/user/join/**", "/api/user/auth/**", "/api/files/**", "/api/board/notice", "/api/reservation/open", "/api/user/phone/**", "/api/board/popular").permitAll()	// 로그인, 회원가입, 권한검증 경로는 인증없이 호출 가능
